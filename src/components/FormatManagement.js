...
const FormatManagement = () => {
  ...
  const fetchFormats = useCallback( async () => {
    ...
    const response = await fetch('http://localhost:4000/api/format/get', {
      headers: {
  ...
  const handleSubmit = async () => {
    ...
    const url = currentFormat.id ? `http://localhost:4000/api/format/${method.toLowerCase()}/${currentFormat.id}` : `http://localhost:4000/api/format/${method.toLowerCase()}/`;
  ...
  const handleDelete = async (id) => {
    const token = await getAccessTokenSilently();
    await fetch(`http://localhost:4000/api/format/delete/${id}`, {
      method: 'DELETE',
      headers: {
        Authorization: `Bearer ${token}`,
      },
